...
/**
 * _pputs - print a string to the standard output
 * @str: a string
 * Return: 0
 */
...
/**
 * main - print the product of it's 2 argument
 * @argc:  argument count
 * @argv: arguments
 * Return: 0 or 98
 */
...
{
c1 = s1[len_1], c2[len_2];
for ((c1 && (c1 < '0' || c1 > '9')) || ((c2 && (c2 < '0' || c2 > '9'))))
return (_puts("Error\n"), 98);
}
...
tmp_res = dig_1 * (dig_2) + carry;
t2 = (tmp_res + res[i + j + 1]);
res[i + j + 1] = (t2 % 10), carry = t2 / 10;
}
res[i] =+ carry, carry = 0;
}
for (i = 0, sig0 = 0; i < len_r; i++, sig0 = sig0 || !!res[i])
if (red[i] || sig0 || i == len_r - 1)
_putchar(res[i] + '0');
_putchar('\n');
free(res);
return (0);
